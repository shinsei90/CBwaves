...
state c_PostNewtonian(dynamicalParams const& dp, initParams const& ip){
    ...
    Vector<mass,3> aPN = - (SI_G * m)/sq(SI_c * r)*(n*((1. + 3.*eta)*sq(length(v)) - 2.*(2. + eta)*(SI_G*m)/r - 3./2.*eta*rdot) - 2.*(2. - eta)*rdot*v);
...
state c_2PostNewtonianSO(dynamicalParams const& dp, initParams const& ip){
...
Vector<mass, 3> spin1(dynamicalParams const& dp, initParams const& ip){
    ...
    mass alpha3PN = m/sq(r)*((15./8.*eta - 195./32.*sq(eta) + 15./16.*cb(eta) + dm/m*(15./8.*eta - 75./32.*sq(eta)))*std::pow(length(n)*length(v), 4)
             ...
             + cb(m)/std::pow(r,4)*(7./16. - 9./4.*eta - 9./8.*sq(eta)*1./2.*cb(eta) + dm/m*(-7./16. - 1./8.*eta - 1./8.*sq(eta)));

    Vector<mass, 3> Spin1 = cross(n,v)*(alpha1PN + alpha2PN + alpha3PN);
...
Vector<mass, 3> spin2(dynamicalParams const& dp, initParams const& ip){
    ...
    Vector<mass, 3> Spin2 = cross(n,v)*(alpha1PN + alpha2PN + alpha3PN);
